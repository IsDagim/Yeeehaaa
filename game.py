...
SPAWN_DELAY_MIN, SPAWN_DELAY_MAX = 1, 2.0
SCALE_GLOBAL = 0.2
FRAME_RATE = 0.1  
...
class Jetpack(arcade.Window):
    ...
    def on_update(self, dt: float):
        ...
        now = time.time()
        if now - self.last_spawn_time > random.uniform(SPAWN_DELAY_MIN, SPAWN_DELAY_MAX):
            x = WIDTH + 100
            y = FIXED_Y
            speed = -random.uniform(SPEED_MIN, SPEED_MAX)
            self.walkers.append([x, y, random.randrange(len(walk_p)), speed, now])
